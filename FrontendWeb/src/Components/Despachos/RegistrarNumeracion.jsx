    ...
    const handleRegistrar = () => {
        ...
        setTimeout(() => {
            try {
                ...
                // PA HU16-3 / 3.1: habilitar la etapa operativa según el canal
                try {
                    if (!canal) throw new Error('canal no disponible');
                    setErrorEtapa('');
                } catch {
                    setErrorEtapa('Numeración registrada pero ocurrió un error al actualizar la etapa. Contacta al administrador');
                }
            } catch {
